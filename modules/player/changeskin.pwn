...
hook OnPlayerPickUpDynPickup(playerid, pickupid)
{
    if(pickupid == gPickupID && !gIsModelSelectionVisible[playerid] && gPlayerTickCount[playerid] < GetTickCount())
    {
        ...
        ShowModelSelectionMenu(playerid, skinlist, "Select a skin", 0x00000046, 0x6FA3FF99, 0x9ABEFFAA);
    }
}
	...
	if(listid == skinlist)
	{
		if(response)
		{
            if(GetPlayerCash(playerid) < CLOTHES_PRICE)
            {
                ...
                SendClientMessagef(playerid, COLOR_ERROR, "* You do not have enough money. ($%d)", CLOTHES_PRICE);
            }
            else
            {
                SetPlayerSkin(playerid, modelid);
                GivePlayerCash(playerid, -CLOTHES_PRICE);
                SendClientMessagef(playerid, COLOR_SUCCESS, "* You changed skin. (-$%i)", CLOTHES_PRICE);
