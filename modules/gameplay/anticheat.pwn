...
public OnCheatDetected(playerid, ip_address[], type, code)
{
    if(!type)
    {
        ...
        if(count)
            SendAdminMessage(PLAYER_RANK_RECRUIT, COLOR_LIGHT_RED, "[ANTICHEAT] The player %s was accused of suspicions of %s.", GetPlayerNamef(playerid), g_anticheat_names[code]);
        else
        {
            SendClientMessageToAllf(COLOR_LIGHT_RED, "[ANTICHEAT] The player %s was kicked from the server. Reason: suspicions of %s.", GetPlayerNamef(playerid), g_anticheat_names[code]);
            Kick(playerid);
        }
    }
    return 1;
