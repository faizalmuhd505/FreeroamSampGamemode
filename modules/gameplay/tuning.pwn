...
*
* DESCRIÇÃO :
*       Permite jogadores Tuning seus veículos em qualquer lugar
*
* NOTAS :
*       -
*/
...
static colorMenu[]      = "Enter the color you want to paint the vehicle:\nSeparate the colors by a space, example: 1 126.";
...
YCMD:Tuning(playerid, params[], help)
{
    if(!IsPlayerInAnyVehicle(playerid))
        return SendClientMessage(playerid, COLOR_ERROR, "* You need to be in a vehicle to tun.");

    else if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER)
        return SendClientMessage(playerid, COLOR_ERROR, "* You need to be the driver to tune the vehicle.");

    ShowPlayerDialog(playerid, DIALOG_TUNING, DIALOG_STYLE_LIST, "Tuning", mainMenu, "Confirm", "X");
                ...
                switch (listitem)
                {
                    case 0:
                    {
                        if(!IsValidPaintJobVehicle(GetPlayerVehicleID(playerid)))
                        {
                            ...
                            ShowPlayerDialog(playerid, DIALOG_TUNING, DIALOG_STYLE_LIST, "Tuning", mainMenu, "Confirm", "X");
                            SendClientMessage(playerid, COLOR_ERROR, "* It is not possible to apply paintjob in this vehicle.");
                        }
                        else
                        {
                            PlayConfirmSound(playerid);
                            ShowPlayerDialog(playerid, DIALOG_TUNING_PAINTJOB, DIALOG_STYLE_LIST, "Tuning", paintjobMenu, "Confirm", "X");
                        }
                    }
                    case 1:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_COLOR, DIALOG_STYLE_INPUT, "Tuning", colorMenu, "Confirm", "X");
                    }
                    case 2:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_EXHAUST, DIALOG_STYLE_LIST, "Tuning", exhaustMenu, "Confirm", "X");
                    }
                    case 3:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_FBUMP, DIALOG_STYLE_LIST, "Tuning", fbumpMenu, "Confirm", "X");
                    }
                    case 4:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_RBUMP, DIALOG_STYLE_LIST, "Tuning", rbumpMenu, "Confirm", "X");
                    }
                    case 5:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_ROOF, DIALOG_STYLE_LIST, "Tuning", roofMenu, "Confirm", "X");
                    }
                    case 6:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_SPOILER, DIALOG_STYLE_LIST, "Tuning", spoilerMenu, "Confirm", "X");
                    }
                    case 7:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_SKIRT, DIALOG_STYLE_LIST, "Tuning", skirtMenu, "Confirm", "X");
                    }
                    case 8:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_BULLBAR, DIALOG_STYLE_LIST, "Tuning", bullbarMenu, "Confirm", "X");
                    }
                    case 9:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_WHEEL, DIALOG_STYLE_LIST, "Tuning", wheelMenu, "Confirm", "X");
                    }
                    case 10:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_NITRO, DIALOG_STYLE_LIST, "Tuning", nitroMenu, "Confirm", "X");
                    }
                    case 11:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_HOOD, DIALOG_STYLE_LIST, "Tuning", hoodMenu, "Confirm", "X");
                    }
                    case 12:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_VENT, DIALOG_STYLE_LIST, "Tuning", ventMenu, "Confirm", "X");
                    }
                    case 13:
                    {
                        PlayConfirmSound(playerid);
                        ShowPlayerDialog(playerid, DIALOG_TUNING_LIGHT, DIALOG_STYLE_LIST, "Tuning", lightMenu, "Confirm", "X");
                    }
                    case 14:
                    {
        ...
        case DIALOG_TUNING_COLOR:
        {
            ...
            else
            {
                new color1, color2;
             	...
             	{
                    PlayErrorSound(playerid);
                    SendClientMessage(playerid, COLOR_ERROR, "* Invalid format.");
                    ShowPlayerDialog(playerid, DIALOG_TUNING_COLOR, DIALOG_STYLE_INPUT, "Tuning", colorMenu, "Confirm", "X");
                }
                else
                {
                    ...
                    case 0:
                    {
                        switch (GetVehicleModel(GetPlayerVehicleID(playerid)))
                        {
                            ...
                            default:
                            {
                                PlayErrorSound(playerid);
                                ShowPlayerDialog(playerid, DIALOG_TUNING_EXHAUST, DIALOG_STYLE_LIST, "Tuning", exhaustMenu, "Confirm", "X");
                                SendClientMessage(playerid, COLOR_ERROR, "* It is not possible to add this component in this vehicle.");
                                return 1;
                            }
                        }
                        PlayBuySound(playerid);
                    ...
                    case 1:
                    {
                        switch (GetVehicleModel(GetPlayerVehicleID(playerid)))
                        {
                            ...
                            default:
                            {
                                PlayErrorSound(playerid);
                                ShowPlayerDialog(playerid, DIALOG_TUNING_EXHAUST, DIALOG_STYLE_LIST, "Tuning", exhaustMenu, "Confirm", "X");
                                SendClientMessage(playerid, COLOR_ERROR, "* X.");
                                return 1;
                            }
                        }
                        PlayBuySound(playerid);
