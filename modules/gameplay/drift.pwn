...
*
* DESCRIÇÃO :
*	   Permite que os drifts do jogadores gerem POINTS.
*
* NOTAS :
*	   -
*/
...
public OnDriftUpdate(playerid, Float: drift_angle, Float: speed)
{
    ...
    {
        new points[32];
        format(points, sizeof(points), "POINTS:~n~%d", gPlayerPoints[playerid]);
...
public OnDriftEnd(playerid, reason, Float: distance, time)
{
    ...
    if(reason == DRIFT_END_REASON_CRASH)
    {
        ...
        GameTextForPlayer(playerid, "~r~it hit", 1500, 3);
    }
    ...
    */
    ...
    {
        new tmpstr[32];
        format(tmpstr, sizeof(tmpstr), "time: %02d second", time / 1000);
        PlayerTextDrawSetString(playerid, gDriftPointsPlayerTextDraw[playerid][1], tmpstr);
        format(tmpstr, sizeof(tmpstr), "POINTS: %02d", gPlayerPoints[playerid]);
        PlayerTextDrawSetString(playerid, gDriftPointsPlayerTextDraw[playerid][2], tmpstr);
        format(tmpstr, sizeof(tmpstr), "distance: %.2f", distance);
...
HidePlayerDriftTextdraw(playerid, bool:hide_best_score = false)
{
    ...
    PlayerTextDrawSetString(playerid, gPlTextDrift[playerid][0], "POINTS:~n~0");
...
hook OnPlayerConnect(playerid)
{
    ...
    gDriftPointsPlayerTextDraw[playerid][1] = CreatePlayerTextDraw(playerid, 471.698608, 300.515686, "time:_0");
    ...
    gDriftPointsPlayerTextDraw[playerid][2] = CreatePlayerTextDraw(playerid, 471.698608, 310.515686, "POINTS:_0");
    ...
    gDriftPointsPlayerTextDraw[playerid][3] = CreatePlayerTextDraw(playerid, 471.698608, 320.515686, "distance:_0");
    ...
    // 2nd drift hud
    gPlTextDrift[playerid][0] = CreatePlayerTextDraw(playerid, 316.000213, 111.599952, "POINTS:~n~0");
    ...
    gPlTextDrift[playerid][1] = CreatePlayerTextDraw(playerid, 603.499938, 0.055507, "TOTAL_SCORE:_");
...
GetPlayerDriftPoints(playerid)
{
    return gPlayerTotalScore[playerid];
}

SetPlayerDriftPoints(playerid, value)
{
    new tmpstr[32];
