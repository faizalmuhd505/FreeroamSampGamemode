/*******************************************************************************
* Name DO ARQUIVO :		modules/gameplay/interior.pwn
*
* DESCRIÇÃO :
*	   Adiciona o comando /Interior para Teleport.
*
* NOTAS :
*	   -
*/
...
YCMD:Interior(playerid, params[], help)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_INTERIOR_LIST, DIALOG_STYLE_TABLIST_HEADERS, "Interior", output, "Teleport", "X");
    return 1;
}
        ...
        case DIALOG_INTERIOR_LIST:
        {
            if(response)
            {
                if(GetPlayerGamemode(playerid) == GAMEMODE_FREEROAM)
                {
                    ...
                    SendClientMessagef(playerid, COLOR_SUCCESS, "* You teleported to %s.", gInteriorData[listitem][e_interior_name]);
                    PlaySelectSound(playerid);
                }
                else
                {
                    PlayErrorSound(playerid);
                    SendClientMessage(playerid, COLOR_ERROR, "*You cannot use this command in this game mode.");
                }
            }
            else
            {
