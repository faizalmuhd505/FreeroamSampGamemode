    ...
    switch (dialogid)
    {
        case DIALOG_DERBY:
        {
            ...
            else if(GetDerbyPlayerPoolSize(listitem) == GetDerbyMaxPlayers())
            {
                ...
                SendClientMessage(playerid, COLOR_ERROR, "* This derby is packed.");
                ShowPlayerDerbyList(playerid);
            }
            else if(gDerbyData[listitem][e_derby_state] == DERBY_STATE_STARTED)
            {
                SendClientMessage(playerid, COLOR_INFO, "* This derby has already started, you will be by viewer until the next match.");
...
public OnPlayerEnterDerby(playerid, derbyid)
{
    ...
    if(gDerbyData[derbyid][e_derby_state] == DERBY_STATE_WAITING_PLAYERS || gDerbyData[derbyid][e_derby_state] == DERBY_STATE_STARTING)
    {
        ...
        SendClientMessagef(playerid, COLOR_SUCCESS, "* You entered the derby %s!", gDerbyData[derbyid][e_derby_name]);
    }
        ...
        if(count >= MINIMUM_PLAYERS_TO_START_DERBY)
        {
            foreach(new i: Player)
            {
                if(GetPlayerDerby(i) == derbyid)
                {
                    ...
                    SendClientMessagef(i, COLOR_SUCCESS, "* The minimum amount of players has been reached, the derby will start in %d second.", DERBY_COUNT_DOWN);
                }
            }
        ...
        else if(gDerbyData[derbyid][e_derby_state] == DERBY_STATE_STARTING)
        {
            if(gDerbyCountdown[derbyid] > 0)
            {
                foreach(new i: Player)
                {
                    ...
                    {
                        new countstr[38];
                        format(countstr, sizeof(countstr), "~g~Starting Derby~n~%02d", gDerbyCountdown[derbyid]);
            ...
            else
            {
                ...
                foreach(new i: Player)
                {
                    ...
                    {
                        PlayerPlaySound(i, 1056, 0.0, 0.0, 0.0);
                        GameTextForPlayer(i, "~g~~h~Derby will start~n~prepare", 1250, 3);
        ...
        else if(gDerbyData[derbyid][e_derby_state] == DERBY_STATE_STARTED)
        {
            ...
            else
            {
                ...
                foreach(new i: Player)
                {
                    if(GetPlayerDerby(i) == derbyid)
                    {
                        if(gPlayerData[i][e_state] == DERBY_PLAYER_STATE_ALIVE)
                        {
                            ...
                            {
                                if(away_time > 4500)
                                    SendClientMessage(i, COLOR_WARNING, "* You were disqualified for being absent in Derby.");
                ...
                if(remaining_players < 2)
                {
                    ...
                    new output[512];
                    strcat(output, "#\tPlayer\tTimePlayed\n");
                    new ranking = 0;
                    ...
                    foreach(new i: Player)
                    {
                        if(GetPlayerDerby(i) == derbyid)
                        {
                            ShowPlayerDialog(i, DIALOG_DERBY_LEADERBOARD, DIALOG_STYLE_TABLIST_HEADERS, "FINAL RESULTS", output, "Close", "");
                        }
                    }
...
ShowPlayerDerbyList(playerid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_DERBY, DIALOG_STYLE_TABLIST_HEADERS, "Derby", output, "Select", "X");
}
