...
public OnPlayerEnterBuilding(playerid, building)
{
    ...
    {
        SendClientMessage(playerid, COLOR_ERROR, "* Locked door.");
        return 0;
    }
    return 1;
}
...
YCMD:abuildingcmds(playerid, params[], help)
{
    if(!IsPlayerAdmin(playerid))
 		return SendClientMessage(playerid, COLOR_ERROR, "* You need to be logged in to this.");

	SendClientMessage(playerid, COLOR_TITLE, "---------------------------------------- Buildings - Database Commands ----------------------------------------");
...
YCMD:buildingid(playerid, params[], help)
{
    ...
    if(building_id == INVALID_BUILDING_ID)
        SendClientMessage(playerid, COLOR_ERROR, "* You are not close to a building.");
    else
...
{
    if(!IsPlayerAdmin(playerid))
        return SendClientMessage(playerid, COLOR_ERROR, "* You need to be logged in to this for this.");
    ...
    if(bid == INVALID_BUILDING_ID)
        return SendClientMessage(playerid, COLOR_ERROR, "* The limit of buildings has been reached.");
    ...
    SendClientMessagef(playerid, COLOR_ADMIN_ACTION, "* You inserted a buildings in the database, IDB: %d.", bid);
...
YCMD:updatebuilding(playerid, params[], help)
{
	...
	if(sscanf(params, "is[8]", bid, option))
		SendClientMessage(playerid, COLOR_INFO, "* /updatebuilding [ID] [entry - saida - door]");
    else if(bid < 0 || bid > MAX_BUILDINGS-1)
        SendClientMessagef(playerid, COLOR_ERROR, "* Invalid building, values from 0 to %i.", MAX_BUILDINGS);
    else if(gBuildingData[bid][e_building_db] == 0)
        SendClientMessage(playerid, COLOR_ERROR, "* This building does not exist.");
    else if(!strcmp(option, "entry"))
    {
        ...
        SendClientMessagef(playerid, COLOR_ADMIN_ACTION, "* You changed the entrance to the building %d.", bid);
    ...
    else if(!strcmp(option, "exit"))
    {
        ...
        SendClientMessagef(playerid, COLOR_ADMIN_ACTION, "*You changed the output of the building %d.", bid);
    ...
    else if(!strcmp(option, "lock"))
    {
        if(gBuildingData[bid][e_building_locked])
        {
            ...
            SendClientMessagef(playerid, COLOR_ADMIN_ACTION, "* You unlock the building %d.", bid);
        }
        else
        {
            gBuildingData[bid][e_building_locked] = 1;
            SendClientMessagef(playerid, COLOR_ADMIN_ACTION, "* You locked the building %d.", bid);
        }
        new query[60];
    ...
    else SendClientMessage(playerid, COLOR_ERROR, "* Invalid option.");
    return 1;
}
...
YCMD:deletebuilding(playerid, params[], help)
{
    ...
    {
        SendClientMessagef(playerid, COLOR_ADMIN_ACTION, "* You deleted the building %d.", bid);
